    ...
    GT_evaluator <- function(annotations){
        library(dplyr)
        gt_o <- read.csv(paste0(WORK_DIR, "manual_curate.csv"))[, -2]
        gt <- filter(gt_o, Confidence == "TRUE") %>% filter(., Background...13 != "Background")
        # annotations <- read.csv(ann_address)
        gt_anns <- annotations[which(annotations$input_id %in% gt$Index), ] %>% arrange(., input_id)
        sub_gt <- gt[which(gt$Index %in% annotations$input_id), ] %>% arrange(., Index)
        judge <- gt_anns$formula == sub_gt$Ground.truth
        correct <- sum(judge)/length(judge)
        fdr <- FDR_Calculator(NULL, NULL, 1, gt_anns$formula)
        return(c(correct, fdr))
    }
    
    GetInfo_1 <- function(x_o) {
        x = filter(x_o, formula != "Unknown")
        # if(nrow(x) == 0) return(list(formula = "Unknown", score = 0.))
        if(nrow(x) == 0) return(list(formula = "Unknown", score = 0., ilp_node_id = x_o$ilp_node_id[1]))
        inds <- which(x$score_mz == max(x$score_mz))
        ind <- sample(inds, 1)
        formulas <- x$formula[ind]
        mz_scores <- x$score_mz[ind]
        # info <- list(formula = formulas, score = mz_scores)
        info <- list(formula = formulas, score = mz_scores, ilp_node_id = x$ilp_node_id[ind])
        return(info)
    }  
    
    GetInfo_2 <- function(x_o) {
        x = filter(x_o, formula != "Unknown")
        # if(nrow(x) == 0) return(list(formula = "Unknown", score = 0.))
        if(nrow(x) == 0) return(list(formula = "Unknown", score = 0., ilp_node_id = x_o$ilp_node_id[1]))
        inds <- which(x$cplex_score == max(x$cplex_score))
        ind <- sample(inds, 1)
        formulas <- x$formula[ind]
        mz_scores <- x$score_mz[ind]
        # info <- list(formula = formulas, score = mz_scores)
    ...
    ScareCrow <- function(work_dir) {
        library(dplyr)
        setwd(work_dir)
        nodes <- read.csv("ilp_nodes.csv")[, -1]
        edges <- read.csv("ilp_edges.csv")[, -1]
        ni_anns <- read.csv("annotations.csv") 
        # ni_anns <- ni_anns$x
        anns <- filter(ni_anns, formula != "Unknown")
        # nodes <- CplexSet$ilp_nodes
        # edges <- CplexSet$ilp_edges
        node_id <- unique(nodes$node_id)
        by_nodes <- split(nodes, nodes$node_id)
        input_id <- sapply(by_nodes, function(x) unique(x$Input_id))
        sc1 <- sapply(by_nodes, GetInfo_1)
        sc2 <- sapply(by_nodes, GetInfo_2)
        sc3 <- matrix(0, nrow = 3, ncol = length(node_id))
        rownames(sc3) = c("formula", "score", "ilp_node_id")
        colnames(sc3) = node_id
        for(id in node_id) {
            e1 <- edges[which(edges$node1 == id), c(1, 4, 11, 18)]
            e2 <- edges[which(edges$node2 == id), c(2, 5, 12, 18)]
            colnames(e1) = c("ilp", "node", "formula", "score")
            colnames(e2) = c("ilp", "node", "formula", "score")
            e = rbind(e1, e2)
            if(nrow(e) == 0) {
                # sc3[[id]] <- list(formulas = "Unknown", score = 0)
                sc3[1, id] = "Unknown"
                sc3[2, id] = 0.
                sc3[3, id] = 0
                # print(id)
            ...
            if(nrow(e) == 1) {
                # sc3[[id]] <- list(formulas = e$formula, score = e$score)
                sc3[1, id] = e$formula
                sc3[2, id] = e$score
                sc3[3, id] = e$ilp
                next
            ...
            by_edges <- sapply(split(e$score, list(e$node, e$ilp)), function(x) sum(x) / 2)
            formulas <- sapply(split(e$formula, list(e$node, e$ilp)), unique)
            if(!identical(names(formulas), names(by_edges))) stop("ERROR!")
            ind <- which(by_edges == max(by_edges)) %>% sample(., 1)
            # sc3[[id]] <- list(formulas = unname(formulas[ind]), mz_scores = unname(by_edges[ind]))
            sc3[2, id] = unname(by_edges[ind])
            sc3[1, id] = unname(formulas[ind])
            sc3[3, id] = as.numeric(unlist(strsplit(names(formulas[ind]), "\\."))[2])
            
            # ilp_ids <- matrix(unlist(strsplit(names(by_edges), "\\.")), nrow = 2)[2, ] %>% as.numeric()
        }
        # fdr0 <- FDR_Calculator(NULL, NULL, 1, ni_anns)
        anns1 <- FDR_Adapter(as.character(sc1[1, ]), as.numeric(sc1[2, ]), 
                            nrow(ni_anns) - nrow(anns), input_id)
        anns2 <- FDR_Adapter(as.character(sc2[1, ]), as.numeric(sc2[2, ]), 
                             nrow(ni_anns) - nrow(anns), input_id)
        anns3 <- FDR_Adapter(as.character(sc3[1, ]), as.numeric(sc3[2, ]), 
                             nrow(ni_anns) - nrow(anns), input_id)
        sc1[1, ] = anns1$formula
        sc2[1, ] = anns2$formula
        sc3[1, ] = anns3$formula
        solution1 <- nodes$ilp_node_id %in% sc1[3, ]
        solution2 <- nodes$ilp_node_id %in% sc2[3, ]
        solution3 <- nodes$ilp_node_id %in% sc3[3, ]
        nodes_p <- nodes
        nodes_p$mz_solution = solution1
        nodes_p$node_solution = solution2
        nodes_p$edge_solution = solution3
        write_csv(nodes_p, "nodes_p.csv")
        fdr0 <- FDR_Calculator(NULL, NULL, 1, ni_anns[, 2])
        fdr1 <- FDR_Calculator(NULL, NULL, 1, anns1[, 2])
        fdr2 <- FDR_Calculator(NULL, NULL, 1, anns2[, 2])
        fdr3 <- FDR_Calculator(NULL, NULL, 1, anns3[, 2])
        g0 <- GT_evaluator(ni_anns)
        g1 <- GT_evaluator(anns1)
        g2 <- GT_evaluator(anns2)
        g3 <- GT_evaluator(anns3)
        sta <- matrix(c(fdr0, fdr1, fdr2, fdr3, 
                        g0[1], g1[1], g2[1], g3[1],
                        g0[2], g1[2], g2[2], g3[2]), 
                      ncol = 4, byrow = T)
        colnames(sta) = c("ni", "mz", "node", "edge")
        rownames(sta) = c("TD", "GTTD", "TDinGT")
        return(sta)
        # return(c(fdr0, fdr1, fdr2, fdr3))
    }
    
    FDR_Adapter <- function(forms, scores, n_unknown, input_id) {
        n_unknown = n_unknown - length(which(forms == "Unknown"))
        if(n_unknown < 0) stop("ERROR!")
        r <- range(scores)
        step <- seq(from = r[1], to = r[2], length.out = 1000)
        n_by_step <- sapply(step, function(x) abs(length(which(scores < x)) - n_unknown))
        adapter <- step[which(n_by_step == min(n_by_step))[1]]
        forms[which(scores < adapter)] = "Unknown"
        return(data.frame(input_id = input_id, formula = forms, score = scores))
        # return(FDR_Calculator(NULL, NULL, 1, forms))
        
    }
    
    Target_Decoy_Generator <- function(ratio, times, lib_ref) {
        address <- vector()
        H_mass = 1.00782503224
        e_mass = 0.00054857990943
        proton_mz <- round(H_mass - e_mass, 4)
        # target_o <- read.csv("D:/NetIDtest4.0/NetID/dependent/ymdb_library.csv")
        target_o <- read.csv(paste0(WORK_DIR, lib_ref, "_library.csv")) #"D:/NetIDtest4.0/NetID/dependent/pbcm_library2.csv")
        target <- target_o[sapply(unique(target_o$formula), function(x) which(target_o$formula == x)[1]), ]
        known_lib <- read.csv(paste0(WORK_DIR, "known_library.csv"))
        elems_target <- unique(unlist(sapply(c(target$formula, known_lib$formula), function(x) unlist(strsplit(x, "\\d+")))))
        # f1=my_calculate_formula("C1H2","He1")
        # formula_mz(f1)
        implausible <- "He, Li, Be, B, C, N, O, F, Ne, Mg, Al, Si, P, S, Cl, Ar, Ca, Sc, Ti, V, Cr, Mn, Fe, Co, Ni, Cu, Zn, Ga, Ge, As, Se, Br, Kr, Rb, Sr, Y, Zr, Nb, Mo, Ru, Rh, Pd, Ag, Cd, In, Sn, Sb, Te, I, Xe, Cs, Ba, La, Ce, Pr, Nd, Sm, Eu, Gd, Tb, Ho, Er, Tm, Yb, Lu, Hf, Ta, W, Re, Os, Ir, Pt, Au, Hg, Tl, Pb, Bi, Th, U, Dy, Pu"
        imp_elements <<- unlist(strsplit(implausible, ", ", fixed = T)) %>% "["(., c(-2, -26, -80, -81))  # remove Cu, Li, Dy, Pu
        imp_elements <<- imp_elements[!(imp_elements %in% elems_target)]
        mass <- sapply(imp_elements, 
                       function(x) elem_table$mass[which(x == elem_table$element)])
        
        ratio = ratio # the # of target over # of decoy
        set.seed(233)
        rd <- ceiling(runif(ceiling(nrow(target) / ratio) * times, min = 0, max = length(mass))) %>% matrix(., nrow = times, byrow = T)
        dir.create(paste0(WORK_DIR, "decoys"))
        for(i in 1 : times) {
            srd <- mass[rd[i, ]]
            elems <- paste(names(srd), "1H-1", sep = '')
            smp <- sample(1 : nrow(target), size = ceiling(nrow(target) / ratio))
            decoy <- target[smp, ]
            # decoy$mass = decoy$mass - proton_mz + unname(srd)
            decoy$formula = mapply(my_calculate_formula, decoy$formula, elems)
            decoy$mass = sapply(decoy$formula, formula_mz)
            decoy$accession = paste0(decoy$accession, "_d")
            
            # if seperate decoy and target
            # db <- decoy
            # if merge decoy and target
            db <- rbind(decoy, target_o) %>% arrange(., accession)
            apd <- as.character(i)
            if(nchar(i) == 1) apd = paste0('0', apd)
            apd = paste0('_', apd)
            write_csv(db, paste(WORK_DIR, "decoys/", lib_ref,"_decoy", apd, ".csv", sep = ''))
            # write_csv(db, paste("D:/NetIDtest4.0/NetID/dependent/decoys/", "ymdb_decoy_", i, ".csv", sep = ''))
            address = c(address, paste(WORK_DIR, "decoys/", lib_ref, "_decoy", apd, ".csv", sep = ''))
            # address = c(address, paste("D:/NetIDtest4.0/NetID/dependent/decoys/", "ymdb_decoy_", i, ".csv", sep = ''))
            print(paste(i, "decoy written."))
        }
        print("OVER.")
        return(address)
    }
    
    FDR_Calculator <- function(decoy_address, output_address, 
                               ratio, annotations = NULL) {
        f = 1 + ratio
        # imp_elements <- read.csv("D:/NetIDtest4.0/NetID/dependent/pbcm_imp_elements.csv")
        imp_elements <- read.csv(paste0(WORK_DIR, "imp_elements.csv"))
        imp_elements <- imp_elements$x
        if(is.null(annotations)) {
            ni_output <- read.csv(output_address)
            annotations = ni_output$formula
        } 
        # annotations = annotations$formula
        anns <- annotations[which(annotations != "Unknown")]
    ...
    NetID <- function(work_dir, file_name, ion_mode, decoy_lib) {
        ...
        read_files = function(
            ...
            ion_mode = -1,
            neg_MS2_library_file = "D:/NetIDtest4.0/NetID/dependent/HMDB_pred_MS2_neg.rds",
            pos_MS2_library_file = "D:/NetIDtest4.0/NetID/dependent/HMDB_pred_MS2_pos.rds",
            # HMDB_library_file = "../dependent/hmdb_library.csv",
            HMDB_library_file = "D:/NetIDtest4.0/NetID/dependent/pbcm_library1.csv",
            known_library_file = "D:/NetIDtest4.0/NetID/dependent/known_library.csv",
            LC_method = "Hilic_25min_QE",
            manual_library_file = "manual_library.csv",
            empirical_rule_file = "D:/NetIDtest4.0/NetID/dependent/empirical_rules.csv",
            propagation_rule_file = "D:/NetIDtest4.0/NetID/dependent/propagation_rule.csv"
        ){
        ...
        # Read data and files ####
        {
            Mset = list()
            # Read in files 
            Mset = read_files(filename = file_name,
                              LC_method = "Hilic_25min_QE", # "Hilic_Rutgers_QEPlus" "Hilic_25min_QE", lipids is empty
                              ion_mode = ion_mode, # 1 for pos mode and -1 for neg mode
                              HMDB_library_file = decoy_lib
            )
        ...
        # Run optimization ####
        {
            ...
            write.csv(CplexSet$ilp_nodes, paste0(work_dir, "/ilp_nodes.csv"))
            write.csv(CplexSet$ilp_edges, paste0(work_dir, "/ilp_edges.csv"))
            annotations <- data.frame(input_id = CplexSet$ilp_nodes$Input_id[which(CplexSet$ilp_nodes$ilp_solution == 1)],
                                      formula = CplexSet$ilp_nodes$formula[which(CplexSet$ilp_nodes$ilp_solution == 1)])
            return(annotations)
        }
        ...
        # 
        #     formula1 = rep(c("C1H2C1",'Psd1C2H3'),1)
        #     linktype = rep(c("[13]C1C-1","[29]Si-1[30]Si1"),1)
        #     test = log10(mapply(isotopic_abundance, formula1, linktype))
        #     test2 = mapply(isotopic_abundance, formula1, linktype)
        #     str(test2)
        # 
        #     test = data.frame(x=c(1,2),y=c(2,3)) %>%
        #         mutate(v =1 ) %>%
        #         mutate(test = log10(mapply(isotopic_abundance, formula1, linktype)))
        # }
        # 
        # save.image()
    ...
    NetID_Go <- function(work_dir, file_name, ion_mode, decoy_lib) {
        gc()
        annotations <- NetID(work_dir, file_name, ion_mode, decoy_lib)
        return(annotations)
        
    }
    
    NetID_wrapper <- function(ratio, times, decoy) {
        # decoy_lib_list <- Target_Decoy_Generator(ratio, times)
        decoy_lib_list <- dir(paste0(WORK_DIR, "decoys"), #  pattern = "hmdb_decoy_\\d+.csv"
                              pattern = paste0(decoy, "_\\d+.csv"), full.names = T)[1 : times]
        work_dir_list <- paste0(WORK_DIR, sapply(strsplit(basename(decoy_lib_list), "\\."), function(x) x[[1]][1]))
        # work_dir_list <- paste0("D:/NetIDtest4.0/NetID/Sc_neg/", substr(basename(decoy_lib_list), 1, 14))
        file_name <- paste0(WORK_DIR, "raw_data.csv")
        ion_mode <- -1
        # FDR <- vector(length = length(decoy_lib_list))
        for(i in 1 : length(work_dir_list)) {
            # gc()
            if(dir.exists(work_dir_list[i])) next
            dir.create(work_dir_list[i])
            print(work_dir_list[i])
            # Sys.sleep(30)
            tryCatch({
                annotations <- NetID_Go(work_dir_list[i], file_name, ion_mode, decoy_lib_list[i])
                write_csv(annotations, paste0(work_dir_list[i], "/annotations.csv"))
                # FDR[i] = FDR_Calculator(decoy_lib_list[i], 
                #                         paste0(work_dir_list[i], "/NetID_output.csv"), 
                #                         ratio, annotations)
                print(paste(i, "out of", times, "finished."))
            }, error = function(e) {
                # file.rename(from = work_dir_list[i], to = paste0(work_dir_list[i], "_u"))
                print(paste("Failed", work_dir_list[i]))
                cat(paste0(work_dir_list[i], "\n"), file = "D:/NetIDtest4.0/NetID/Sc_neg/log", append = T)
            }, finally = {
                Sys.sleep(30)
                gc()
            })
        ...
        cat(paste0('--------------------', '\n'), file = paste0(WORK_DIR, "log"), append = T)
        print("OVER!")
        return(T)
    }
    
    ScareCrow_Repeater <- function(decoy) {
        dir_list <- dir(WORK_DIR, pattern = paste0(decoy, "_\\d+"), full.names = T)
        mm <- list()
        for(i in 1 : length(dir_list)) {
            dir <- dir_list[i]
            mm[[dir]] <- ScareCrow(dir)
        }
        return(mm)
    }
